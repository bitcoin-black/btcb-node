	...
	if (!init_error ())
	{
		...
		if ((network_params.network.is_live_network () || network_params.network.is_beta_network ()) && !flags.inactive_node)
		{
			...
			if (use_bootstrap_weight)
			{
				...
				{
					logger.always_log ("Using bootstrap rep weight: ", rep.first.to_account (), " -> ", nano::uint128_union (rep.second).format_balance (Mbcb_ratio, 0, true), " Btcb");
				}
			}
...
{
	debug_assert (balance_a >= amount_a * nano::Gbcb_ratio);
